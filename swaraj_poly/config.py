...
MIN_KELLY  = float(os.getenv("MIN_KELLY", "0.02"))   # ↑ raised from 0.005; break-even ~1.5%
MIN_VOLUME = float(os.getenv("MIN_VOLUME", "5000"))

# ── Timing ───────────────────────────────────────────────────────────────────
SCAN_INTERVAL = int(os.getenv("SCAN_INTERVAL", "900"))    # 15 min between scans
POLL_INTERVAL = int(os.getenv("POLL_INTERVAL", "60"))     # fill-check loop cadence

# ── Order execution ──────────────────────────────────────────────────────────
ORDER_FILL_TIMEOUT = int(os.getenv("ORDER_FILL_TIMEOUT", "300"))   # 5 min to fill or cancel
RETRY_MAX          = int(os.getenv("RETRY_MAX", "3"))               # API retry attempts

# ── Gas guardrail ────────────────────────────────────────────────────────────
MIN_MATIC_GAS = float(os.getenv("MIN_MATIC_GAS", "0.05"))   # abort live if < 0.05 MATIC
